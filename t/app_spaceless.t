...
subtest 'cmd.exe' => sub {
  ...
  note capture_stdout { map { $_->mkpath(1,oct('700')) } $dir1, $dir2 };
  ok -d $dir1, "dir $dir1";
  ok -d $dir2, "dir $dir2";
...
subtest 'actual spacelessness' => sub {
  ...
  note capture_stdout { $_->mkpath(1, oct('700')) for @foo };
...
subtest 'trim' => sub {
  ...
  $tmp->subdir('dir1')->mkpath(0,oct('700'));
...
subtest '--no-cygwin' => sub {
  ...
  $_->mkpath(0,oct('700')) for map { $tmp->subdir($_) } qw( dir1 dir2 dir3 );
  note "FOO=$ENV{FOO}";
